...
public class MultiblockStructureDefinition{
	public class BlockDec{
		...
		public BlockDec(int blockID, int meta){
		...
		public String toString() {
			...
			return String.format("(%s) ID: %d, meta: %d", blockName, blockID, meta);
		}

		@Override
		public boolean equals(Object obj) {
			if (obj instanceof BlockDec){
				return this.blockID == ((BlockDec)obj).blockID && (this.meta == -1 || ((BlockDec)obj).meta == -1 || this.meta == ((BlockDec)obj).meta);
			}
			return false;
		}
	...
	public class StructureGroup
	{
		...
		void addAllowedBlock(int offsetX, int offsetY, int offsetZ, int blockID, int meta){
			...
			positionReplacements.add(new BlockDec(blockID, meta));
		}
		...
		boolean matchGroup(World world, int originX, int originY, int originZ){
			for (BlockCoord offset : allowedBlocks.keySet()){
				int blockID = world.getBlockId(originX + offset.x, originY + offset.y, originZ + offset.z);
				...
				for (BlockDec bd : positionReplacements){
					if (bd.blockID == blockID && (bd.meta == -1 || bd.meta == meta)){
						valid = true;
					}
				}
		...
		public void replaceAllBlocksOfType(int originalBlockID, int newBlockID){
			replaceAllBlocksOfType(originalBlockID, -1, newBlockID, -1);
		}

		public void replaceAllBlocksOfType(int originalBlockID, int originalMeta, int newBlockID, int newMeta){
			for (BlockCoord bc : allowedBlocks.keySet()){
				for (BlockDec bd : allowedBlocks.get(bc)){
					if (bd.blockID == originalBlockID){
						if (bd.meta == originalMeta || originalMeta == -1){
							bd.blockID = newBlockID;
							if (newMeta != -1){
								bd.meta = newMeta;
							}
						}
	...
	public void addAllowedBlock(int offsetX, int offsetY, int offsetZ, int blockID, int meta){
		...
		mainGroup.addAllowedBlock(offsetX, offsetY, offsetZ, blockID, meta);
	}

	public void addAllowedBlock(int offsetX, int offsetY, int offsetZ, int blockID){
		addAllowedBlock(offsetX, offsetY, offsetZ, blockID, -1);
	}

	public void addAllowedBlock(StructureGroup group, int offsetX, int offsetY, int offsetZ, int blockID, int meta){
		...
		group.addAllowedBlock(offsetX, offsetY, offsetZ, blockID, meta);
	}

	public void addAllowedBlock(StructureGroup group, int offsetX, int offsetY, int offsetZ, int blockID){
		addAllowedBlock(group, offsetX, offsetY, offsetZ, blockID, -1);
	}
	...
	public StructureGroup copyGroup(String originalName, String destinationName, int newMutex){
		...
		for (BlockCoord bc : copyGroup.allowedBlocks.keySet()){
			for (BlockDec bd : copyGroup.allowedBlocks.get(bc)){
				newGroup.addAllowedBlock(bc.x, bc.y, bc.z, bd.blockID, bd.meta);
			}
		}
